...
// MySQL Example:
// Build DSN dynamically. If host is 'localhost' prefer XAMPP socket when available,
// otherwise fall back to TCP (127.0.0.1) or the configured host.
$dbHost = $config['database']['host'] ?? 'localhost';
$dbName = $config['database']['dbname'] ?? '';
$xamppSocket = '/opt/lampp/var/mysql/mysql.sock';
if ($dbHost === 'localhost') {
	if (file_exists($xamppSocket)) {
		// Use XAMPP unix socket for local XAMPP installations
